...
template<typename EncoderType, uint8_t PacketMarker = ' ', size_t BufferSize = 256>
class PacketSerial_
{
public:
        ...
        while (_stream->available() > 0)
        {
            ...
            if (data == PacketMarker)
            {
                if (_onPacketFunction || _onPacketFunctionWithSender)
                {
                    ...
                    size_t numDecoded;
                    if(_receiveBufferIndex == 1 && _receiveBuffer[0] == '0') {
                      numDecoded = 1;
                      _decodeBuffer[0] = '0';
                    } else {
                      numDecoded = EncoderType::decode(_receiveBuffer,
                                                      _receiveBufferIndex,
                                                      _decodeBuffer);
                    }
